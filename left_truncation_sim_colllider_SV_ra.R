...
surv_sim <- function(iteration, s_e, lb_e, u_s_e, u_lb_e, N) {
  ...
  dat$U <- rbinom(n=N, size=1, prob=0.3)            
  ...
  dat$spont <- ifelse(dat$A==1, rbinom(n=N, size=1, prob = dat$sa_risk + s_e + u_s_e*dat$U),
                      rbinom(n=N, size=1, prob = dat$sa_risk + u_s_e*dat$U))
  ...
  dat$lb_time <- ifelse(dat$A==1, dat$lb_time_u - lb_e - u_lb_e*dat$U, 
                               dat$lb_time_u - u_lb_e*dat$U)
  ...
  dat$ptb <- ifelse(dat$time<37 & dat$event_type=="live birth", 1, 
  ...
  #rd_ffit_u <- glm(event ~ A + SV + U, data=dat)
...
mc <- function(n, s_e, lb_e, u_s_e, u_lb_e, N) {
ls <- lapply(1:n, function(x) surv_sim(x, s_e = s_e, lb_e = lb_e, u_s_e=u_s_e, u_lb_e = u_lb_e, N = N))
df <- do.call(rbind, ls)
r <- colMeans(df)
return(r)
}
...
surv_ls <- lapply(seq(0, 0.2, by=0.01), function(y) 
                    lapply(seq(-0.05, 0.05, by=0.05), function(z) mc(n = 1000, s_e = y, lb_e = z, u_s_e=0.2, u_lb_e=0.5, N=625000)))
...
write.csv(surv_m, file="/Users/danagoin/Documents/Research projects/Left truncation of birth cohorts simulation study/results/sim_results_collider_SV_ra_1000.csv", row.names = F)

surv_m <- read.csv("/Users/danagoin/Documents/Research projects/Left truncation of birth cohorts simulation study/results/sim_results_collider_SV_ra_1000.csv")
